...
class LigerDPOLossComputer:
    ...
    def compute_loss(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        attention_mask = batch.get("attention_mask")
        moe_metrics = collect_moe_metrics(outputs, top_k=self.aux_loss_computer.num_experts_per_tok, attention_mask=attention_mask)
    ...
    def compute_loss(
        ...
        *,
        ...
        metrics = self.metrics_aggregator.build_training_metrics(
            ...
            moe_attention_mask=model_attention_mask,
        )
