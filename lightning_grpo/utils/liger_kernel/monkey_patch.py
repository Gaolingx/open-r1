...
def apply_liger_kernel_to_nekomind_moe(
    rope: bool = True,
    cross_entropy: bool = False,
    fused_linear_cross_entropy: bool = True,
    rms_norm: bool = True,
    ...
    from lightning_grpo.module.nekomind.nekomind_moe import modeling_nekomind_moe
    from lightning_grpo.module.nekomind.nekomind_moe.modeling_nekomind_moe import NekoMindMoeModel

    from lightning_grpo.utils.liger_kernel.model.nekomind_moe import lce_forward as nekomind_lce_forward
    from liger_kernel.transformers.swiglu import LigerQwen3MoeSwiGLUMLP

    if rope:
        modeling_nekomind_moe.apply_rotary_pos_emb = liger_rotary_pos_emb

    if rms_norm:
        modeling_nekomind_moe.NekoMindMoeRMSNorm = LigerRMSNorm
    ...
    if fused_linear_cross_entropy:
        if model is not None:
            model.forward = MethodType(nekomind_lce_forward, model)
        else:
            modeling_nekomind_moe.NekoMindMoeForCausalLM.forward = nekomind_lce_forward

    if swiglu:
        if IS_TRANSFORMERS_V5_OR_LATER:
            modeling_nekomind_moe.NekoMindMoeExperts = LigerExperts
        else:
            modeling_nekomind_moe.NekoMindMoeMLP = LigerQwen3MoeSwiGLUMLP
    ...
    if model is not None:
        ...
        # get the base model from the model instance
        base_model: NekoMindMoeModel = getattr(model, model.base_model_prefix, model)
