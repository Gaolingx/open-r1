"""CLI entrypoint for Lightning-based Direct Preference Optimization training."""
...
from lightning_grpo.data.dpo_datamodule import DPODataModule
from lightning_grpo.models.dpo_module import DPOLightningModule
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train a DPO model with PyTorch Lightning.")
...
def main() -> None:
    """Load config, construct trainer objects, and launch DPO training."""

    args = parse_args()
    config = load_experiment_config(args.config)
    if not isinstance(config, DPOConfig):
        raise TypeError("Expected a DPO config for train_dpo.py")
    ...
    module = DPOLightningModule(config)
