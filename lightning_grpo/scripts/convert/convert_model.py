...
from .model_lora import apply_lora, merge_lora
...
def convert_torch2transformers_minimind(torch_path, transformers_path, dtype=torch.float16):
    ...
    MiniMindMoeForCausalLM.register_for_auto_class("AutoModelForCausalLM")
    lm_model = MiniMindMoeForCausalLM(lm_config)
    ...
    # ======= Compatibility handling for lower versions in transformers 5.0 =======
...
# Compatible with the QwenForCausalLM/LlamaForCausalLM ecosystem
...
def convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    lm_model = MiniMindMoeForCausalLM(lm_config).to(device)
    state_dict = torch.load(base_torch_path, map_location=device)
    lm_model.load_state_dict(state_dict, strict=False)
    apply_lora(lm_model)
    merge_lora(lm_model, lora_path, merged_torch_path)
    print(f"LoRA has been merged and saved in base-model PyTorch format: {merged_torch_path}")
...
def convert_json_to_jinja(json_file_path, output_path):
    ...
    print(f"Template saved as a Jinja file: {output_path}")
